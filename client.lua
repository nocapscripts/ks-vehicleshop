...
local function createTestDriveReturn()
    ...
    testDriveZone:onPlayerInOut(function(isPointInside)
        if isPointInside and IsPedInAnyVehicle(PlayerPedId()) then
            ...
            exports['el-menu']:openMenu(returnTestDrive)
        else
            exports['el-menu']:closeMenu()
        end
    end)
end
    ...
    if not Config.UsingTarget then
        ...
        combo:onPlayerInOut(function(isPointInside)
            if isPointInside then
                if PlayerData and PlayerData.job and (PlayerData.job.name == Config.Shops[insideShop]['Job'] or Config.Shops[insideShop]['Job'] == 'cardealer') then
                    exports['el-menu']:showHeader(vehHeaderMenu)
                end
            else
                exports['el-menu']:closeMenu()
            end
            ...
            options = {
                {
                    ...
                    canInteract = function()
                        local closestShop = insideShop
                        return closestShop  --(Config.Shops[closestShop]['Job'] == PlayerData.job or PlayerData.job.name == Config.Shops[closestShop]['Job'])
                    end
                },
            },
            distance = 3.0
...
function Init()
    ...
    CreateThread(function()
        ...
        financeZone:onPlayerInOut(function(isPointInside)
            if isPointInside then
                exports['el-menu']:showHeader(financeMenu)
            else
                exports['el-menu']:closeMenu()
            end
        end)
...
RegisterNetEvent('qb-vehicleshop:client:homeMenu', function()
    exports['el-menu']:openMenu(vehicleMenu)
end)
...
RegisterNetEvent('qb-vehicleshop:client:vehCategories', function()
    ...
    exports['el-menu']:openMenu(categoryMenu)
end)
...
RegisterNetEvent('qb-vehicleshop:client:openVehCats', function(data)
    ...
    exports['el-menu']:openMenu(vehMenu)
end)
...
RegisterNetEvent('qb-vehicleshop:client:getVehicles', function()
    QBCore.Functions.TriggerCallback('qb-vehicleshop:server:getVehicles', function(vehicles)
        ...
        if #ownedVehicles > 0 then
            exports['el-menu']:openMenu(ownedVehicles)
        else
...
RegisterNetEvent('qb-vehicleshop:client:getVehicleFinance', function(data)
    ...
    exports['el-menu']:openMenu(vehFinance)
end)
